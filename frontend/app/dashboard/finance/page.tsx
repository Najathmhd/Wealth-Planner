...
import { PlusCircle, Save, Trash2, Loader2, AlertCircle, CheckCircle, Zap } from "lucide-react"
        ...
        <div className="flex-1 space-y-4">
            ...
            <Tabs defaultValue="income" className="space-y-4">
                <TabsList className="bg-black/20 border border-white/10">
                    ...
                    <TabsTrigger value="currency" className="data-[state=active]:bg-primary data-[state=active]:text-white">Currency</TabsTrigger>
                </TabsList>
...
function CurrencyConverter() {
    ...
    const convert = async () => {
        setLoading(true);
        try {
            const res = await api.get(`/finance/convert?amount=${amount}&from_curr=${from}&to_curr=${to}`);
            setResult(res.data);
        } catch (e) {
    ...
    return (
        <Card className="glass border-white/10">
            <CardHeader>
                <CardTitle className="text-white">Global Currency Converter</CardTitle>
                <CardDescription>Real-time exchange rates for international wealth planning.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
                ...
                <Button onClick={convert} disabled={loading} className="w-full bg-secondary hover:bg-secondary/90 text-black font-bold">
                    {loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Zap className="mr-2 h-4 w-4" />}
                    Convert Now
                </Button>

                {result && (
                    <div className="p-6 rounded-xl bg-white/5 border border-white/10 flex flex-col items-center justify-center space-y-2">
                        <p className="text-sm text-white/60">Exchange Rate: 1 {from} = {result.rate} {to}</p>
                        <p className="text-4xl font-bold text-white tracking-tighter">
                            {result.currency} {result.converted_amount?.toLocaleString()}
                        </p>
                        <p className="text-xs text-emerald-400 flex items-center gap-1">
                            <CheckCircle className="h-3 w-3" /> Live Rate via {result.source}
                        </p>
                    </div>
                )}
            </CardContent>
